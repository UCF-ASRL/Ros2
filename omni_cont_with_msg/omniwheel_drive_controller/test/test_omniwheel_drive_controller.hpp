...
#ifndef TEST_OMNIWHEEL_DRIVE_CONTROLLER_HPP_
#define TEST_OMNIWHEEL_DRIVE_CONTROLLER_HPP_
...
#include "omniwheel_drive_controller/omniwheel_drive_controller.hpp"
...
using ControllerStateMsg = omniwheel_drive_controller::OmniwheelDriveController::ControllerStateMsg;
using ControllerReferenceMsg =
  omniwheel_drive_controller::OmniwheelDriveController::ControllerReferenceMsg;
using TfStateMsg = omniwheel_drive_controller::OmniwheelDriveController::TfStateMsg;
using OdomStateMsg = omniwheel_drive_controller::OmniwheelDriveController::OdomStateMsg;
...
class TestableOmniwheelDriveController : public omniwheel_drive_controller::OmniwheelDriveController
{
  FRIEND_TEST(OmniwheelDriveControllerTest, when_controller_is_configured_expect_all_parameters_set);
  FRIEND_TEST(
    OmniwheelDriveControllerTest, when_controller_configured_expect_properly_exported_interfaces);
  FRIEND_TEST(OmniwheelDriveControllerTest, when_controller_is_activated_expect_reference_reset);
  FRIEND_TEST(OmniwheelDriveControllerTest, when_controller_active_and_update_called_expect_success);
  FRIEND_TEST(OmniwheelDriveControllerTest, when_active_controller_is_deactivated_expect_success);
  FRIEND_TEST(
    OmniwheelDriveControllerTest,
    when_controller_is_reactivated_expect_cmd_itfs_not_set_and_update_success);
  FRIEND_TEST(OmniwheelDriveControllerTest, when_update_is_called_expect_status_message);
  FRIEND_TEST(
    OmniwheelDriveControllerTest,
    when_reference_msg_received_expect_updated_commands_and_status_message);
  FRIEND_TEST(OmniwheelDriveControllerTest, when_reference_msg_is_too_old_expect_unset_reference);
  FRIEND_TEST(
    OmniwheelDriveControllerTest,
    when_reference_msg_has_timestamp_zero_expect_reference_set_and_timestamp_set_to_current_time);
  FRIEND_TEST(OmniwheelDriveControllerTest, when_message_has_valid_timestamp_expect_reference_set);
  FRIEND_TEST(
    OmniwheelDriveControllerTest,
...
public:
  ...
  {
    return omniwheel_drive_controller::OmniwheelDriveController::on_configure(previous_state);
  }
    ...
    const rclcpp_lifecycle::State & previous_state) override
  {
    ...
    return omniwheel_drive_controller::OmniwheelDriveController::on_activate(previous_state);
  }
...
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_omniwheel_drive_controller/reference", rclcpp::SystemDefaultsQoS());
    ...
    odom_s_publisher_ = odom_s_publisher_node_->create_publisher<OdomStateMsg>(
      "/test_omniwheel_drive_controller/odometry", rclcpp::SystemDefaultsQoS());
    ...
    tf_odom_s_publisher_ = tf_odom_s_publisher_node_->create_publisher<TfStateMsg>(
      "/test_omniwheel_drive_controller/tf_odometry", rclcpp::SystemDefaultsQoS());
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_omniwheel_drive_controller")
  {
    const auto urdf = "";
    const auto ns = "";
    ASSERT_EQ(
  ...
  void subscribe_to_controller_status_execute_update_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_omniwheel_drive_controller/controller_state", 10, subs_callback);
...
#endif  // TEST_OMNIWHEEL_DRIVE_CONTROLLER_HPP_
