...
controller_interface::CallbackReturn OmniwheelDriveController::on_init()
{
  try
  {
    param_listener_ = std::make_shared<omniwheel_drive_controller::ParamListener>(get_node());
  }
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  ref_subscriber_ = get_node()->create_subscription<ControllerReferenceMsg>(
    ...
    std::bind(&OmniwheelDriveController::reference_callback, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(get_node()->get_logger(), "OmniwheelDriveController configured successfully");

  return controller_interface::CallbackReturn::SUCCESS;
}

void OmniwheelDriveController::reference_callback(const std::shared_ptr<ControllerReferenceMsg> msg)
{
...
OmniwheelDriveController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration OmniwheelDriveController::state_interface_configuration()
  const
{
...
bool OmniwheelDriveController::on_set_chained_mode(bool chained_mode)
{
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  {
    velocity_in_center_frame_linear_x_ = reference_interfaces_[0];
    velocity_in_center_frame_linear_y_ = reference_interfaces_[1];
    velocity_in_center_frame_angular_z_ = reference_interfaces_[2];

    const double wheel_front_left_vel =
      ((sin(0.7854) * velocity_in_center_frame_linear_x_) - (cos(0.7854) * velocity_in_center_frame_linear_y_) - (params_.kinematics.sum_of_robot_center_projection_on_X_Y_axis * velocity_in_center_frame_angular_z_))/params_.kinematics.wheels_radius;
    const double wheel_rear_left_vel =
      ((sin(2.3562) * velocity_in_center_frame_linear_x_) - (cos(2.3562) * velocity_in_center_frame_linear_y_) - (params_.kinematics.sum_of_robot_center_projection_on_X_Y_axis * velocity_in_center_frame_angular_z_))/params_.kinematics.wheels_radius;
    const double wheel_rear_right_vel =
      ((sin(3.9270) * velocity_in_center_frame_linear_x_) - (cos(3.9270) * velocity_in_center_frame_linear_y_) - (params_.kinematics.sum_of_robot_center_projection_on_X_Y_axis * velocity_in_center_frame_angular_z_))/params_.kinematics.wheels_radius;
    const double wheel_front_right_vel =
      ((sin(5.4978) * velocity_in_center_frame_linear_x_) - (cos(5.4978) * velocity_in_center_frame_linear_y_) - (params_.kinematics.sum_of_robot_center_projection_on_X_Y_axis * velocity_in_center_frame_angular_z_))/params_.kinematics.wheels_radius;
...
PLUGINLIB_EXPORT_CLASS(
  omniwheel_drive_controller::OmniwheelDriveController,
  controller_interface::ChainableControllerInterface)
