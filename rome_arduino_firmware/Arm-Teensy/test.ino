...
const int LIMIT_SWITCH_HIGH[] = {
    0, 0, 0, 0, 0, 0};  // to account for both NC and NO limit switches
const int CAL_DIR[] = {-1, 1, -1,
                       -1, 1, 1};  // joint rotation direction to limit switch.1234 {-1, 1, -1,-1, 1, 1}
const int CAL_SPEED = 500;          // motor stepspawner_joint_state_broadcasters per second 
const float CAL_SPEED_MULT[] = {
    1, 0.5, 0.5, 1, 0.5, 1};  // multiplier to account for motor steps/rev
...
void setup() {
  ...
  for (int i = 0; i < NUM_JOINTS; ++i) {
    ...
    limitSwitches[i].attach(LIMIT_PINS[i], INPUT_PULLUP);
