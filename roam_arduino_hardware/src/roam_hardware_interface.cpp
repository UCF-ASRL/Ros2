#include "roam_arduino_hardware/roam_arduino_interface.hpp"
...
hardware_interface::return_type ArduinoInterface::write(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) {
  ...
  try {
    // Get the motor velocity commands
  ...
  // Log the motor commands for debugging purposes
  RCLCPP_INFO(rclcpp::get_logger("arduino_actuator_interface"), "Motor1 command: %.2f, Direction: %d", velocity_commands_.at(0), (velocity_commands_.at(0) >= 0) ? 0 : 1);
  RCLCPP_INFO(rclcpp::get_logger("arduino_actuator_interface"), "Motor2 command: %.2f, Direction: %d", velocity_commands_.at(1), (velocity_commands_.at(1) >= 0) ? 0 : 1);
  RCLCPP_INFO(rclcpp::get_logger("arduino_actuator_interface"), "Motor3 command: %.2f, Direction: %d", velocity_commands_.at(2), (velocity_commands_.at(2) >= 0) ? 0 : 1);
  RCLCPP_INFO(rclcpp::get_logger("arduino_actuator_interface"), "Motor4 command: %.2f, Direction: %d", velocity_commands_.at(3), (velocity_commands_.at(3) >= 0) ? 0 : 1);
...
hardware_interface::return_type ArduinoInterface::read(const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
    ...
    //I dont really care about the position component
    // Log position and velocity states for all joints
    RCLCPP_INFO(rclcpp::get_logger("arduino_actuator_interface"), "Motor 1 Velocity: %.2f", velocity_states_[0]);
    RCLCPP_INFO(rclcpp::get_logger("arduino_actuator_interface"), "Motor 2 Velocity: %.2f", velocity_states_[1]);
    RCLCPP_INFO(rclcpp::get_logger("arduino_actuator_interface"), "Motor 3 Velocity: %.2f", velocity_states_[2]);
    RCLCPP_INFO(rclcpp::get_logger("arduino_actuator_interface"), "Motor 4 Velocity: %.2f", velocity_states_[3]);

    //CHANGE THIS BASED ON OUR MOTORS --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // Constants for calculating translational velocity V_t?
    //double R = 0.0762;  // Radius of wheel
    ...
    //double omega_3 = velocity_commands_[2] * (2 * M_PI / 60.0);  // RPM to rad/s
    //double omega_4 = velocity_commands_[3] * (2 * M_PI / 60.0);  // RPM to rad/s

    // Calculate V_t (translational velocity in meters per second)
    //double V_t = -(R / 2) * (omega_1 - omega_2);
